...
│   ├── snakeoil.csr
│   ├── snakeoil.pem
│   ├── snakeoil_ca-config.json
│   ├── snakeoil_ca-csr.json
│   ├── snakeoil_ca-key.pem
│   ├── snakeoil_ca.csr
│   └── snakeoil_ca.pem
├── compose.yaml
...
```
$ sudo cp certs/snakeoil_ca.pem /usr/local/share/ca-certificates/nginx-cfssl_snakeoil_ca.pem
$ ls -l /usr/local/share/ca-certificates/nginx-cfssl_snakeoil_ca.pem
-rw-r--r-- 1 root root 2033  2月 11 00:51 /usr/local/share/ca-certificates/nginx-cfssl_snakeoil_ca.pem
...
「ローカル証明書」→「カスタム」→「自分でインストール」→「信頼できる証明書」から `certs/snakeoil_ca.pem` をインポートします
