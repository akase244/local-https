...
│   ├── snakeoil.csr
│   ├── snakeoil.pem
│   ├── snakeoil_Development_Root_CA-config.json
│   ├── snakeoil_Development_Root_CA-csr.json
│   ├── snakeoil_Development_Root_CA-key.pem
│   ├── snakeoil_Development_Root_CA.csr
│   └── snakeoil_Development_Root_CA.pem
├── compose.yaml
├── default.conf
├── entrypoint.sh
└── html
...
「ローカル証明書」→「カスタム」→「自分でインストール」→「信頼できる証明書」から `certs/snakeoil_Development_Root_CA.pem` をインポートします
